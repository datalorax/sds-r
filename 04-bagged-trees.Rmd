...
As you can see, we have **substantially** cut the fitting time down because we've only fit the model once. We went from `r tic_seconds(1) / 60` minutes to only `r tic_seconds(2)` seconds! But do we get the same estimates for our metrics if we use the OOB samples? Let's look at the model object
